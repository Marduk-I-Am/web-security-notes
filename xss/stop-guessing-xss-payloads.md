# Stop Guessing XSS Payloads: Identify Context in 3 Steps

![Header image](img/stop-guessing-xss-payloads.png)

## Overview

This article builds directly on my previous write-up, [*Context Is Everything: A Practical Guide to XSS*](https://github.com/Marduk-I-Am/web-security-notes/blob/main/xss/context-is-everything.md), where we broke down how different XSS contexts work and how they affect what payloads are possible.
...
- Is it between tags as visible text? **HTML body**.
    - See [Work-through 1](#work-through-1)
    ```html
...
- Is it inside a tag, within an attribute value? **Attribute context**.
    ...
    - See [Work-through 2](#work-through-2)
    ```html
...
- Is it inside a `<script>` block? **JavaScript context**.
    ...
    - See [Work-through 3](#work-through-3)
    ```html
    <script>
      var query = "M4rduk";
      doSearch(query);
...
- If you see `<i>M4rduk</i>` unchanged:
    ```html
    <p>There are 0 search results for <i>M4rduk</i></p>
    ```
